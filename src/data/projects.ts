...
export const projects: Project[] = [
    {
        ...
        sheets: [
            ...
            },
            {
                ...
                title: "스플래시 — 다크 · 라이트",
                note: "앱이 그리는 스플래시도 테마를 따라 두 벌입니다. 로고는 선 색이 흰색과 남색으로 달라서 색만 바꾸지 않고 PNG를 테마마다 따로 둡니다. 다크의 지면 #080D16이 이 페이지의 지면이기도 합니다.",
            },
        ],
        links: {},
    },
    ...
    },
    {
        ...
        approach: [
            "번호를 고르는 규칙은 한 함수에만 둡니다. 직접 입력이든 엑셀이든 들어온 글을 줄 단위 문자열로 맞춘 뒤 같은 파서를 태웁니다. 입구마다 규칙을 따로 두면 한쪽에서만 통과하는 번호가 생깁니다.",
        ...
        sieve: {
            ...
            samples: [
                {
                    label: "적는 방식이 제각각",
                    hint: "같은 명단을 사람마다 다르게 적어 옵니다. 띄어 쓴 번호는 공백에서 잘려 들어오지만, 숫자가 10자리에 닿을 때까지 이어 붙여 한 번호로 봅니다.",
                    input: "010-2481-3690\n+82 10 7315 2048\n(010) 5520-1874\n1093321706\n01068012245",
                },
                {
                    label: "중복이 섞인 명단",
                    hint: "010으로 적은 번호와 +82로 적은 번호도 국가번호를 붙인 최종 모양끼리 비교합니다. 앞에서 등록된 번호는 뒤에서 중복이 됩니다.",
                    input: "01024813690\n010-2481-3690\n+82 10 2481 3690\n821024813690\n01073152048",
                },
                {
                    label: "틀린 번호",
        ...
        windowing: {
            ...
            lede: "등록한 번호는 칩으로 보여 주고 칩마다 삭제 버튼이 붙습니다. 전부 그리면 건수만큼 DOM이 늘어서, 창에 보이는 줄만 그리게 했습니다. 건수를 바꾸고 상자 안을 굴려 보세요.",
            ...
            note: "원 프로젝트는 react-window의 FixedSizeList를 썼고, 이 상자는 같은 셈을 라이브러리 없이 옮긴 것입니다. 번호 세 개를 한 줄로 묶은 것도 그대로입니다 — 칩 하나하나를 가상화하면 줄이 어디서 꺾일지가 창 폭에 달려 줄 높이를 고정할 수 없습니다.",
        },
        cases: [
            {
                ...
                approach:
                    "파싱을 별도 함수로 떼어 냈습니다. 공백으로 자른 조각을 숫자가 10자리에 닿을 때까지 이어 붙이고, 82로 시작하는 12자리와 10으로 시작하는 10자리는 010 꼴로 정규화한 뒤 휴대폰 패턴으로 판정합니다. 통과한 번호는 앞자리 0을 국가번호로 바꿔 목록에 올리고, 중복도 이 최종 모양끼리 비교합니다.",
        ...
        sheets: [
            {
                src: smsSheetBrand,
                title: "브랜드 두 벌",
                note: "Modoo SMS는 같은 저장소를 VITE_DOMAIN만 바꿔 빌드한 것입니다. 달라지는 건 상수 파일의 네 값 — 로고 · 하단 로고 · 상호 · 메타 타이틀뿐이고, 배포 경로도 브랜드마다 따로 둡니다.",
            },
            {
                src: smsSheetParts,
                title: "번호를 다루는 조각",
