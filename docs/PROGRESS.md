...
- `bun run build` — `tsc -b` 타입 통과 + 프로덕션 빌드 성공 (Projects 지그재그 리스트 반영 후 재검증).
...
- **GitHub 반영**: 직전 커밋 `4cc389d` (`chore: 읽기 권한을 ask(승인형)와 deny(시크릿)로 분리`) → `origin/main` 푸시 완료. 이번 Projects 재설계는 `commit-push`로 커밋·푸시 진행.
